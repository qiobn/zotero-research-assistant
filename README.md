...
| **Column-Aware Extraction** | Two/multi-column journal PDFs are read left → right column (top-to-bottom within each), fixing the garbled interleaved text that naive extraction produces. Broken extractions — scanned, garbled (replacement/NUL chars), or word-per-line — are gated: skipped and reported in the sync report instead of silently indexed. No OCR. |
